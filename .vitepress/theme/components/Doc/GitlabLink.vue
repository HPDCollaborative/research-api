<script setup>
const { theme } = useData();
const gitlabUrl = computed(() => theme.value.gitlabUrl);
</script>

<template>
  <a v-if="gitlabUrl" :href="gitlabUrl" target="_blank" rel="noopener noreferrer">
    <svg
      class="duration-500 fill-current w-9 h-9 grayscale transition-color hover:grayscale-0"
